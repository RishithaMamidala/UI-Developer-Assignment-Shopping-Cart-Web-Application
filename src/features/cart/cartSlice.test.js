...
describe('cartSlice', () => {
  ...
  it('addToCart with existing productId accumulates quantity', () => {
    const stateWithItem = cartReducer(initialState, addToCart({ product: mockProduct, quantity: 3 }));
    const stateAfterSecondAdd = cartReducer(stateWithItem, addToCart({ product: mockProduct, quantity: 4 }));
  ...
  it('addToCart caps accumulated quantity at MAX_QUANTITY (50)', () => {
    const stateWithItem = cartReducer(initialState, addToCart({ product: mockProduct, quantity: 48 }));
    const stateAfterCap = cartReducer(stateWithItem, addToCart({ product: mockProduct, quantity: 10 }));
    expect(stateAfterCap.items[0].quantity).toBe(50);
  });

  it('updateQuantity sets new quantity', () => {
    const stateWithItem = cartReducer(initialState, addToCart({ product: mockProduct, quantity: 2 }));
  ...
  it('updateQuantity is a no-op when productId not found', () => {
    const stateWithItem = cartReducer(initialState, addToCart({ product: mockProduct, quantity: 2 }));
    const stateUnchanged = cartReducer(stateWithItem, updateQuantity({ productId: 999, quantity: 5 }));
  ...
  it('large-cart edge case: 30 distinct products', () => {
    ...
    for (let i = 1; i <= 30; i++) {
      state = cartReducer(state, addToCart({
        product: { productId: i, title: `Product ${i}`, image: `https://example.com/${i}.jpg`, price: 9.99 },
        quantity: 1,
      }));
    }
